...
// Built on access so a theme change is picked up live (IconWell and others read this too).
const TONE_NAMES = ['neutral', 'primary', 'secondary', 'success', 'warning', 'error', 'info', 'platinum', 'gold', 'plus'];
const buildTone = (key) => {
  const c = theme.colors;
  const map = {
...
export const TONES = new Proxy({}, {
  get: (_, key) => (typeof key === 'string' ? buildTone(key) : undefined),
  has: (_, key) => TONE_NAMES.includes(key),
  ownKeys: () => [...TONE_NAMES],
