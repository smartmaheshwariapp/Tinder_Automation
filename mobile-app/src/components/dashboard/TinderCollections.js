...
function ProfileRow({ item, tab, ownerId, onPress }) {
  ...
  return <Button style={styles.row} onPress={onPress} accessibilityRole="button" accessibilityLabel={`Open ${profile?.name || 'profile'} details`}>
    ...
    <View style={styles.rowCopy}>
      <View style={styles.nameLine}><Text style={styles.name} numberOfLines={1}>{profile?.name || 'Tinder profile'}</Text>{tab === 'chatting' && <Text style={styles.time}>{formatTime(item.lastActivityAt)}</Text>}</View>
      <Text style={styles.subtitle} numberOfLines={2}>{subtitle}</Text>
      <View style={styles.meta}>
        {tab === 'swiped' && <View style={[styles.pill, { backgroundColor: active.tint }]}><Ionicons name={item.action === 'like' ? 'heart' : 'close'} size={10} color={active.color} /><Text style={[styles.pillText, { color: active.color }]}>{item.action === 'like' ? 'LIKED' : 'PASSED'}</Text></View>}
...
function Empty({ tab, loading }) {
  const item = TABS[tab];
  const body = tab === 'swiped' ? 'New likes and passes will appear here.'
    ...
    {!state.data ? <LinearGradient colors={['rgba(255,51,102,0.18)', 'rgba(255,170,128,0.08)', '#201428']} style={styles.connect}>
      ...
      <View style={styles.card}>
        ...
        <ContentTransition transitionKey={tab} style={styles.content}>
          <View style={styles.listHeader}><View><Text style={styles.listTitle}>{fullLabel(tab)}</Text><Text style={styles.listCount}>{entries.length} {entries.length === 1 ? 'profile' : 'profiles'}</Text></View>{entries.length > previewLimit && <Button style={styles.viewAll} onPress={() => { setSelected(null); setOpen(true); }}><Text style={[styles.viewAllText, { color: active.color }]}>View all</Text><Ionicons name="arrow-forward" size={14} color={active.color} /></Button>}</View>
