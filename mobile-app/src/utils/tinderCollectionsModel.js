...
export function normalizeProfile(raw) {
  ...
  if (!id) return null;
  return { id, name: text(profile.name, 100) || 'Tinder profile', bio: text(profile.bio),
    photos: (Array.isArray(profile.photos) ? profile.photos : []).map(item => typeof item === 'string' ? item : item?.url || item?.processedFiles?.[0]?.url).filter(url => typeof url === 'string' && url.startsWith('https://')).slice(0, 3),
    interests: list(profile.user_interests || profile.interests || profile.common_interests), languages: list(profile.languages),
    lookingFor: text(typeof profile.relationship_intent === 'string' ? profile.relationship_intent : profile.relationship_intent?.name || profile.lookingFor, 100) };
}
export function normalizeMessage(raw) {
  const id = text(raw?._id || raw?.id, 150); if (!id) return null;
...
export function compatibility(profile, own, preferences = {}) {
  ...
  return { score, reasons: [...shared.map(value => `Shared interest: ${value}`), ...commonLanguages.slice(0, 1).map(value => `Shared language: ${value}`), ...(sameGoal ? ['Same relationship intention'] : [])], strong: score >= 60, estimated: true };
}
...
export function mergeCollectionEvent(state, event, now = Date.now()) {
  ...
  const addProfile = raw => { const profile = normalizeProfile(raw); if (profile) next.profiles[profile.id] = { ...next.profiles[profile.id], ...profile, seenAt: now }; return profile; };
  ...
  if (event.kind === 'matches') for (const match of (event.matches || []).slice(0, 100)) {
    const profile = addProfile(match.person || match.user); const id = text(match._id || match.id, 150); if (!id || !profile) continue;
    const prior = next.conversations[id] || {}; const messages = (match.messages || []).map(normalizeMessage).filter(Boolean);
  ...
  if (event.kind === 'messages') {
    ...
    if (prior) { const messages = (event.messages || []).map(normalizeMessage).filter(Boolean); const combined = Object.values({ ...Object.fromEntries((prior.messages || []).map(message => [message.id, message])), ...Object.fromEntries(messages.map(message => [message.id, message])) }).sort((a, b) => b.sentAt - a.sentAt).slice(0, 10); next.conversations[id] = { ...prior, messages: combined, lastActivityAt: Math.max(prior.lastActivityAt || 0, combined[0]?.sentAt || 0), observedAt: now }; }
  }
  if (event.kind === 'match_index') { const active = new Set(event.ids || []); for (const [id, conversation] of Object.entries(next.conversations)) if (!active.has(id)) next.conversations[id] = { ...conversation, archived: true, observedAt: now }; }
  next.profiles = bound(next.profiles, 'seenAt'); next.swipes = bound(next.swipes, 'swipedAt'); next.conversations = bound(next.conversations, 'lastActivityAt'); return next;
}
