...
export async function activateCollections(sessionToken) {
  ...
  activation=(async()=>{
    try {
      ...
      await AsyncStorage.setItem(key(own.id),JSON.stringify(data));
...
export function ingestCollectionEvent(event, sessionToken=token) {
  ...
  serial=serial.catch(()=>{}).then(async()=>{
    ...
    try {await AsyncStorage.setItem(key(data.ownerId),JSON.stringify(data));if(run!==generation)return;publish({data,error:null});}
