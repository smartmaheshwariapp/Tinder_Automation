...
try:
    ...
    # Priority 1: Extension service worker (MV3)
    ext_target = next(
        (t for t in targets
    ...
    # Priority 2: Extension background page (MV2 fallback)
    if not ext_target:
        ext_target = next(
            (t for t in targets
    ...
    # Priority 3: Content script context on tinder/bumble page
    # (last resort — chrome.storage available in isolated world but harder to target)
    # We skip this and just return null so safe defaults are used.
