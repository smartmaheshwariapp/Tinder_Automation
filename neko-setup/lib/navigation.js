...
while time.time() - start_time < 30:
    ...
    # 0. If stranded on /faq, redirect to login page
    if '/faq' in url_now:
        faq_bounce_count += 1
        if faq_bounce_count <= 1:
            print('NAVIGATING_TO_HOME_FROM_FAQ', flush=True)
            eval_js(ws, "window.location.href = 'https://tinder.com'")
            time.sleep(3.0)
        else:
            # tinder.com keeps redirecting to /faq, go straight to /app/login
            print('NAVIGATING_TO_LOGIN_DIRECT', flush=True)
            eval_js(ws, "window.location.href = 'https://tinder.com/app/login'")
            time.sleep(4.0)
        continue
