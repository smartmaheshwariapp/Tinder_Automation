...
res = ws.call('Runtime.evaluate', {
    ...
    (function() {
      ...
      var containers = Array.from(document.querySelectorAll('h2, h3, h4, h5, div[class*="header" i], div[class*="title" i], div[class*="label" i]')).map(el => ({
        tag: el.tagName,
        text: el.innerText,
        nextSiblingText: el.nextElementSibling ? el.nextElementSibling.innerText : null,
...
print('SECTIONS:', json.dumps(res.get('result', {}).get('value', []), indent=2))
