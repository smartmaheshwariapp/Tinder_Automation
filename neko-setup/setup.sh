...
if [ -x "$(command -v ufw)" ]; then
  ...
  ufw allow 59000/tcp comment 'Neko WebRTC TCP Mux'
  ufw allow 59000:59099/udp comment 'Neko WebRTC UDP Mux & EPR'
  ufw allow 8088/tcp comment 'Neko WebRTC Legacy TCP'
  ufw allow 52000:52100/udp comment 'Neko WebRTC Legacy UDP'
  ufw reload
elif [ -x "$(command -v firewall-cmd)" ]; then
  echo "[*] Firewalld detected. Opening Neko & Orchestrator ports..."
  firewall-cmd --permanent --add-port=3001/tcp
  firewall-cmd --permanent --add-port=8080/tcp
  firewall-cmd --permanent --add-port=59000/tcp
  firewall-cmd --permanent --add-port=59000-59099/udp
  firewall-cmd --permanent --add-port=8088/tcp
  firewall-cmd --permanent --add-port=52000-52100/udp
  firewall-cmd --reload
else
  echo "[!] No standard firewall (UFW/Firewalld) detected. Please ensure ports 3001/tcp, 8080/tcp, 59000/tcp, and 59000-59099/udp are open in your Cloud Provider's console (Security Groups)."
fi
