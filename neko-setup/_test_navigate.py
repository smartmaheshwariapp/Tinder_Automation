...
ws.call('Page.navigate', {'url': 'https://tinder.com/'})
time.sleep(3)

# Click "I accept" cookie consent if present, then click "Log in"
ws.call('Runtime.evaluate', {'expression': """
(function() {
    var btns = Array.from(document.querySelectorAll('button, a, [role="button"]'));
    var cookieBtn = btns.find(b => (b.innerText||'').toLowerCase().includes('i accept') || (b.innerText||'').toLowerCase().includes('accept'));
    if (cookieBtn) cookieBtn.click();
    
    var loginBtn = btns.find(b => (b.innerText||'').toLowerCase().trim() === 'log in' || (b.innerText||'').toLowerCase().trim() === 'login');
    if (loginBtn) {
        loginBtn.click();
...
ws.close()
print("Navigated to Tinder homepage and clicked Log in")
