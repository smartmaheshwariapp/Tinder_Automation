    ...
    if page:
        ...
        (function() {
          ...
          var backBtn = btns.find(function(b) {
            ...
            return label === 'back' || label.indexOf('go back') !== -1 || label === 'close' || label === 'cancel';
          });
          ...
          if (!backBtn) {
            ...
            if (dialog) {
              ...
              backBtn = dialogBtns.find(function(b) {
                var svg = b.querySelector('svg');
                if (!svg) return false;
                var rect = b.getBoundingClientRect();
                return rect.x < 200 && rect.y < 200;
              });
            }
          }
          ...
          document.dispatchEvent(new KeyboardEvent('keyup', { key: 'Escape', keyCode: 27, code: 'Escape', bubbles: true }));
...
`;
    ...
    require('child_process').exec(`docker cp "${tmpPath}" neko:/tmp/goback_${uniqueId}.py`, (cpErr) => {
      ...
      if (cpErr) {
        require('child_process').exec('docker exec neko xdotool key Escape', () => {
          res.writeHead(200, { 'Content-Type': 'application/json' });
          res.end(JSON.stringify({ success: true }));
        });
        return;
      }
      require('child_process').exec(`docker exec neko python3 /tmp/goback_${uniqueId}.py`, () => {
...
`;
    ...
    const checkScript = `
      (function() {
        ...
        const hasSmsOtpInput = !hasEmailOtpInput && (
          (function() {
        ...
        const hasOtpInput = hasSmsOtpInput || hasEmailOtpInput || (
        ...
        const hasEmailInput = !hasOtpInput && (
          document.querySelector('input[type="email"]') !== null ||
          document.getElementById('email') !== null ||
          document.querySelector('input[name="email"]') !== null ||
          (function() {
            var els = Array.from(document.querySelectorAll('input'));
            if (els.some(function(i) {
              var p = (i.placeholder || i.getAttribute('aria-label') || i.name || i.id || '').toLowerCase();
              return p.indexOf('email') !== -1 || p.indexOf('e-mail') !== -1;
            })) return true;
            var headings = Array.from(document.querySelectorAll('h1, h2, h3, h4, div, span, p, label'));
            return headings.some(function(e) {
              var txt = (e.innerText || e.textContent || '').toLowerCase();
              return txt.indexOf("what's your email") !== -1 || txt.indexOf('enter email') !== -1 || txt.indexOf('verify your email') !== -1 || txt.indexOf('account recovery') !== -1 || txt.indexOf("we'll email you a link") !== -1;
            });
          })()
        );
        ...
        const hasLoginOptions = !hasEmailInput && !hasPhoneInput && (
          (function() {
          ...
          (function() {
            ...
            return els.some(function(e) {
              ...
              return txt.indexOf('wait up to one minute') !== -1 ||
                     ...
                     txt.indexOf('too many requests') !== -1 ||
    ...
    (function() {
        ...
        var hasEmailInput = (
          ...
          (function() {
            ...
            for (var i = 0; i < els.length; i++) {
              var p = (els[i].placeholder || els[i].getAttribute('aria-label') || els[i].name || '').toLowerCase();
              if (p.indexOf('email') !== -1) return true;
            }
            var headings = document.querySelectorAll('h1, h2, h3, h4, div, span, p');
            for (var j = 0; j < headings.length; j++) {
              var txt = (headings[j].innerText || headings[j].textContent || '').toLowerCase();
              if (txt.indexOf('account recovery') !== -1 || txt.indexOf("we'll email you a link") !== -1) {
                return true;
              }
            }
            return false;
