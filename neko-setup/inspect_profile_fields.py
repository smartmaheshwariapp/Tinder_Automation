...
res = ws.call('Runtime.evaluate', {
    'expression': """
    (function() {
      var inputs = Array.from(document.querySelectorAll('input, textarea')).map(el => ({
        tag: el.tagName,
        name: el.name,
        id: el.id,
      ...
      var headings = Array.from(document.querySelectorAll('h1, h2, h3, h4, h5, div[role="heading"]')).map(h => h.innerText);
...
print('PROFILE_FIELDS:', json.dumps(res.get('result', {}).get('value', {}), indent=2))
