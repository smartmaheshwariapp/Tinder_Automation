...
click_iframe = eval_js("""(function(){
    var iframe = document.querySelector('iframe[src*="google"]') || document.querySelector('iframe[src*="accounts.google.com"]');
    if (!iframe) {
        var iframes = Array.from(document.querySelectorAll('iframe'));
        iframe = iframes.find(f => (f.src||'').includes('google') || (f.id||'').includes('gsi'));
    }
    if (iframe) {
        var rect = iframe.getBoundingClientRect();
        return { found: true, src: iframe.src, x: rect.left + rect.width / 2, y: rect.top + rect.height / 2 };
    }
    return { found: false };
})()""")

print("Google Iframe search result:", json.dumps(click_iframe, indent=2))

if click_iframe and click_iframe.get('found') and click_iframe.get('x'):
    gx, gy = click_iframe['x'], click_iframe['y']
    print(f"Dispatching CDP mouse click to iframe center: ({gx}, {gy})")
...
tabs2 = http_get('http://localhost:9222/json')
print("Tabs after clicking iframe:", json.dumps(tabs2, indent=2))
