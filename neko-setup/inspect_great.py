...
(function() {
  ...
  for (var h of el) {
    ...
    var grand = p ? p.parentElement : null;
    var great = grand ? grand.parentElement : null;
    blocks.push({
      h2: h.innerText,
      greatText: great ? great.innerText : ''
    });
  }
  return blocks;
})()
...
res = ws.call('Runtime.evaluate', {'expression': inspect_js, 'returnByValue': True})
for b in res.get('result', {}).get('value', []):
    if any(k in b['h2'] for k in ['PASSIONS', 'HEIGHT', 'RELATIONSHIP', 'LIVING', 'JOB']):
        print('--- BLOCK ---')
        print('H2:', b['h2'])
        print('GREAT_TEXT:', repr(b['greatText']))
