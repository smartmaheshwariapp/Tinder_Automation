...
function updateGeolocationUI() {
  const enabled = document.getElementById('geolocationEnabled')?.checked;
  const settings = document.getElementById('geolocationSettings');
  if (settings) {
    settings.style.opacity = enabled ? '1' : '0.55';
    settings.style.pointerEvents = enabled ? 'auto' : 'none';
    
    settings.querySelectorAll('input, select').forEach(el => {
      if (el.id === 'geolocationCity' && !document.getElementById('geolocationCountry')?.value) {
        el.disabled = true;
      } else {
        el.disabled = !enabled;
      }
...
function initializeGeolocationDropdowns() {
  const countrySelect = document.getElementById('geolocationCountry');
  const citySelect = document.getElementById('geolocationCity');
  const latInput = document.getElementById('geolocationLatitude');
  const lngInput = document.getElementById('geolocationLongitude');

  if (!countrySelect || !citySelect || !latInput || !lngInput) return;

  // 1. Populate Country Dropdown
  Object.keys(GEOLOCATION_CITIES).forEach(country => {
  ...
  // Helper to populate city list
  window.populateCities = function (countryName) {
    citySelect.innerHTML = '<option value="">-- Custom/Select City --</option>';
    if (!countryName || !GEOLOCATION_CITIES[countryName]) {
      citySelect.disabled = true;
      return;
    }
    citySelect.disabled = false;
    Object.keys(GEOLOCATION_CITIES[countryName]).forEach(city => {
      const opt = document.createElement('option');
      opt.value = city;
  ...
  countrySelect.addEventListener('change', () => {
    window.populateCities(countrySelect.value);
    citySelect.value = "";
  });
  ...
  citySelect.addEventListener('change', () => {
    ...
    if (country && city && GEOLOCATION_CITIES[country][city]) {
      const coords = GEOLOCATION_CITIES[country][city];
  ...
  // 4. Handle Lat/Lng manual input: if user types manually, reset selects to custom
  [latInput, lngInput].forEach(input => {
    input.addEventListener('input', () => {
      ...
      let matched = false;
      for (const country in GEOLOCATION_CITIES) {
        for (const city in GEOLOCATION_CITIES[country]) {
          const coords = GEOLOCATION_CITIES[country][city];
          if (Math.abs(coords.lat - lat) < 0.0001 && Math.abs(coords.lng - lng) < 0.0001) {
            if (countrySelect.value !== country) {
              countrySelect.value = country;
              window.populateCities(country);
            }
...
function selectCityByCoords(lat, lng) {
  const countrySelect = document.getElementById('geolocationCountry');
  const citySelect = document.getElementById('geolocationCity');
  ...
  if (!matched) {
    countrySelect.value = "";
    if (typeof window.populateCities === 'function') window.populateCities("");
  }
}

window.initializeGeolocationDropdowns = initializeGeolocationDropdowns;
window.selectCityByCoords = selectCityByCoords;
